...
#define SOJOURN_TARGET          5000000  // 5ms in nsec
#define CONTROL_INTERVAL      100000000 // 100ms in nsec
...
control CoDelEgress (in bit<48> ingress_tstamp, 
	...
	RegisterAction<register_operations, bit<9>, bit<1> >(codel_salu_2) codel_salu_2_action = {
		void apply(inout register_operations inout_vals, out bit<1> out_val){
			...
			if (codel_metadata.first_sojourn_violation == 1w0x1) {
				...
				inout_vals.val2 = 1;
			} else if (codel_metadata.egress_tstamp > inout_vals.val1) {
				//we want to drop
	...
	action a_compute_remainder(){
		codel_metadata.sojourn_remainder = SOJOURN_TARGET |-| codel_metadata.queue_delay; // SOJOURN_TARGET = 5000000
	}
